 ...
 */
public class EnlaceActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
     ...
     String def = "a";
        if(e !=null){
             ...
             def = e.getString("modo");
            if(def.equals("test")){
                Log.d(TAG, "Modo test activado.");
                startAdvertising();
            }else{
                ledStatus = false;
                ...
                handler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if(isInternetConnection()){
                            Log.d(TAG, "Conexion a internet disponible. Lanzando Mainactivity.");
    ...
    private final PayloadCallback mPayloadCallback = new PayloadCallback() {
        @Override public void onPayloadReceived(String endpointId,
            ...
            Log.i(TAG, "Se ha recibido una transferencia desde (" +
                    endpointId + ") con el siguiente contenido: " + message + "WHOA" + partes[0]);
            disconnect(endpointId);
            switch (partes[0]) {
                case "SWITCH":
                    doRemoteAction(partes[1], partes[2]);
                    break;
                default:
    ...
    public void doRemoteAction(String SSID, String contrasenya) {
        Log.d("WTF!!", contrasenya);
        wifiutils.connectToAP(SSID, contrasenya);
